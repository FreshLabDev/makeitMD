...
```sh
docker pull ghcr.io/freshlabdev/makeitmd:<tag>
docker inspect --format '{{index .RepoDigests 0}}' ghcr.io/freshlabdev/makeitmd:<tag>
```

That first pull is not a formality. Deploying straight to
`ghcr.io/freshlabdev/makeitmd@sha256:<digest>` on a host that has never fetched
