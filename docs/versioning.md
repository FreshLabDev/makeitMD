...
makeitMD is pre-`v1.0.0`.

```text
v0.1.0          public MVP release
v0.1.1-alpha.N  current pre-release line
v1.0.0          after the production contract and retention policy settle
```

Pre-releases up to `v0.1.1-alpha.3` were tagged on `main`, under the earlier
rule. From now on they are tagged on `dev`.

makeitMD has no test bot. Its product surface is one message in and one message
out, so a pre-release is usually proven by sending real Markdown to the
production bot — which is the "run a pre-release in production" shortcut, and
should be recorded in the changelog as such.
...
- the shared `core-postgres` contract: the `makeitmd` schema, the
  `makeitmd_core` role, and the `core.touch` call
- migration requirements
- the Bot API version the rich-message path depends on
