...
// Package i18n holds makeitMD's user-facing text. Strings live in
// translations.json (key -> {lang: text}) embedded at build time, and the
// language set mirrors the sibling bots so a person who picked a language in
// one of them is understood by the rest.
//
// T(lang, key, pairs...) returns the localized string with {placeholder}
// interpolation, falling back to English. A language that has not been
// translated yet therefore reads in English rather than going blank, which is
// what lets locales land one at a time.
package i18n
...
// LangOption is a selectable language for the language screen.
type LangOption struct {
	Code  string
	Label string // flag + native name, e.g. "🇬🇧 English"
}

// LANGUAGE_OPTIONS is the order shown in the language picker. It is the same
// list, in the same order, with the same labels, that every bot in the family
// offers: the preference is shared through the core hub, so a picker that
// disagreed with a sibling would offer a language the sibling cannot render.
...
// by another bot to a language makeitMD can render, falling back to English.
